...
- Token saving (strip noise from command outputs, see `token-saver.ts` and `filters.ts`)
- Context pruning (replace old tool results with smart summaries, see `context-pruner.ts`)
- Recurring tasks (run prompts on an interval, see `loop.ts`)
- Stuck detection (abort and steer the agent when stuck in a loop, see `stuck-detection.ts`)
